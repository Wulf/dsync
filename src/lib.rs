    ...
    pub fn apply_defaults(&self, other: &TableOptions<'a>) -> Self {
        Self {
            ...
            autogenerated_columns: self
                ...
                .clone()
                .or(other.autogenerated_columns.clone()),
        }
    }
}
...
pub fn generate_files(
    ...
    if !output_dir.exists() {
        std::fs::create_dir(&output_dir)
            .expect(&format!("Could not create directory '{:#?}'", output_dir));
    ...
    for table in generated.iter() {
        ...
        if !table_dir.exists() {
            std::fs::create_dir(&table_dir)
                .expect(&format!("Could not create directory '{:#?}'", table_dir));
        }
    ...
    for item in std::fs::read_dir(&output_dir)
        .expect(&format!("Could not read directory '{:#?}'", output_dir))
        .into_iter()
    {
        let item = item.expect(&format!("Could not read item in '{:#?}'", output_dir));

        // check if item is a directory
        let file_type = item.file_type().expect(&format!(
            "Could not determine type of file '{:#?}'",
            item.path()
        ));
        if !file_type.is_dir() {
            continue;
        ...
        let associated_table_name = file_name.to_str().expect(&format!(
        ...
        std::fs::remove_file(&generated_rs_path).expect(&format!(
        ...
        let is_empty = item
            .path()
            .read_dir()
            .expect(&format!("Could not read directory {:#?}", item.path()))
            .next()
            .is_none();
        if is_empty {
            std::fs::remove_dir(item.path())
                .expect(&format!("Could not delete directory '{:#?}'", item.path()));
        }
