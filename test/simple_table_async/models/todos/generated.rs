...
impl Todos {
    ...
    pub async fn paginate(db: &mut ConnectionType, page_starting_with_1: i64, page_size: i64, filter: TodosFilter) -> QueryResult<PaginationResult<Self>> {
        use crate::schema::todos::dsl::*;

        let param_page = page_starting_with_1.max(0);
        let page_size = page_size.max(1);
        let total_items = Self::filter(filter.clone()).count().get_result(db)?;
        let items = Self::filter(filter).limit(page_size).offset(param_page * page_size).load::<Self>(db).await?;
